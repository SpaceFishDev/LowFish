...
bool expect(token_type type, parser *Parser, bool error)
{
	...
	if (error)
	{
		...
		sprintf(buffer, msg, token_type_to_string(type),
				token_type_to_string(Parser->tokens[Parser->pos + 1].type));
...
#define expect_err(x)        \
	...
	++Parser->pos
#define expect_err_no_inc(x) expect(x, Parser, true)

#define expect_no_err(x) expect(x, Parser, false)

#define conditional_token(x)                            \
	!strcmp("if", x.text) || !strcmp("else", x.text) || \
		!strcmp("while", x.text) || !strcmp("for", x.text)
...
#define type_token(x)                                         \
	((!strcmp(x.text, "ptr8") || !strcmp(x.text, "ptr") ||    \
	  !strcmp(x.text, "ptr16") || !strcmp(x.text, "ptr32") || \
	  !strcmp(x.text, "struct") || !strcmp(x.text, "i32") ||  \
	  !strcmp(x.text, "i16") || !strcmp(x.text, "i8")) ||     \
	 (!strcmp(x.text, "u32") || !strcmp(x.text, "u16") ||     \
	  !strcmp(x.text, "u8")))
...
node *parse_function_call(parser *Parser)
{
	...
	node *id_node = create_node(FUNCTION_CALL, Parser->tokens[Parser->pos],
								Parser->current_parent);
	expect_err(OPENBR);
...
{
	if (!strcmp("return", Parser->tokens[Parser->pos].text))
	{
		expect_err(ID);
		return create_node(RETURN, Parser->tokens[Parser->pos++], 0);
	}
	if (!strcmp("goto", Parser->tokens[Parser->pos].text))
	{
		expect_err(ID);
		return create_node(GOTO, Parser->tokens[Parser->pos++], 0);
	}
	...
	else if (type_token(Parser->tokens[Parser->pos]))
	{
		...
		{
			// hopefully not a struct definition. (while we're still building
			// out the parser)
			type_node = create_node(
				TYPE, Parser->tokens[Parser->pos + 1],
				0);	 // we just assume it isnt just for testing purposes.
			expect_err(ID);
		}
		if (!expect_no_err(OPENBR))
		{
			node *var_decl =
				create_arbitrary_node(VARDECL, Parser->current_parent);
			node *id =
				create_node(TOKENNODE, Parser->tokens[Parser->pos], var_decl);
...
node *parse_factor(parser *Parser)
{
	...
	while (expect_no_err(DIV) || expect_no_err(MUL) ||
		   expect_no_err(BOOLEQUAL) || expect_no_err(BOOLNOTEQUAL) ||
		   expect_no_err(LESS) || expect_no_err(MORE) ||
		   expect_no_err(MOREEQUAL) || expect_no_err(LESSEQUAL))
	{
		++Parser->pos;
		node *operator=
			create_node(TOKENNODE, Parser->tokens[Parser->pos], left);
		++Parser->pos;
...
node *parse_primary(parser *Parser)
{
	...
	while (expect_no_err(PLUS) || expect_no_err(MINUS) ||
		   expect_no_err(BOOLAND) || expect_no_err(BOOLOR))
	{
		++Parser->pos;
		node *operator=
			create_node(TOKENNODE, Parser->tokens[Parser->pos], left);
...
node *parse_conditional(parser *Parser)
{
	...
	if (!strcmp("else", Parser->tokens[Parser->pos].text))
	{
		...
		{
			// normal else
			if (Parser->current_parent->n_child &&
				Parser->current_parent
						->children[Parser->current_parent->n_child - 1]
						->type == CONDITIONAL)
			{
			...
			put_error("Floating else statement.", 0,
...
node *parse_expression(parser *Parser)
{
	...
	switch (Parser->tokens[Parser->pos].type)
	{
		case LABEL:
		{
			...
			node *n =
				create_node(LABEL_NODE, Parser->tokens[Parser->pos], parent);
		...
		case DEREF:
		{
			node *parent = Parser->current_parent;
			node *id =
				create_node(TOKENNODE, Parser->tokens[Parser->pos], parent);
			if (!expect_no_err(EQUAL))
			{
				return id;
			}
		...
		case ENDOFBLOCK:
		{
			...
			while (p && p->parent && p->type != BLOCK &&
				   p->type != PROGRAM)	//( p && p->parent && p->type != PROGRAM
										//&& !( p->type == BLOCK && p->type !=
										// Parser->current_parent ) )
			{
				p = p->parent;
