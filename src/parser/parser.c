...
node *parse_factor(parser *Parser)
{
    ...
    while (
        expect_no_err(DIV) || expect_no_err(MUL) || expect_no_err(BOOLEQUAL) || expect_no_err(BOOLNOTEQUAL) || expect_no_err(LESS) || expect_no_err(MORE) || expect_no_err(MOREEQUAL) || expect_no_err(LESSEQUAL))
    {
        ++Parser->pos;
