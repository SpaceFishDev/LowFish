...
public:
  ...
  Variable getVariable(std::string name)
  {
    ...
    {
      if (
          v.name == name && v.function == CurrentFunction)
      {
        return v;
      }
    }
  ...
  void CompileIndex16(Node *root)
  {
    if (root->Parent->Type == REFERENCE)
    {
      ...
      int i = 0;
      while (
          i < root->Children.size() && n->Type != INDEXEND)
      {
        n = root->Children[i];
        ++i;
      }
  ...
  void CompileMath16(Node *root)
  {
    if (root->Parent->Type != MATH)
    {
      ...
      if (root->Children[0]->Type == REFERENCE)
      {
        ...
        Text(
            "mov " + std::string((v.size == 1) ? "dl, " : "dx, ") + ((std::string[5]){"", "byte", "word", "", "dword"})[v.size] + "[bp + " + std::to_string(v.stack_pos) + "]");
        if (v.size == 1)
          c1 = "dl, ";
      }
      if (root->Children[1]->Type == REFERENCE)
      {
        ...
        Text(
            "mov " + (v.size == 1) ? "cl, " : "cx, " + ((std::string[5]){"", "byte", "word", "", "dword"})[v.size] + "[bp + " + std::to_string(v.stack_pos) + "]");
        if (v.size == 1)
          c2 = "cl";
      }
      if (
          root->Children[1]->Type == CONSTANT_NODE && root->Children[1]->NodeToken->Type == CONSTANT)
      {
  ...
  void CompileBoolExpr16(Node *root)
  {
    ...
    if (
        root->Parent->Type == IFNODE || root->Parent->Type == WHILENODE)
    {
      if (root->NodeToken->Text == "==")
      {
        std::string c1 = "dx,";
  ...
  {
    if (
        root->Children[0]->Type == CONSTANT_NODE && root->Children[0]->NodeToken->Type == CONSTANT)
    {
  ...
  void CompilePointerVar16(Node *root)
  {
    int sz = 1;
    if (
        root->Children.size() > 0 && root->Children[0]->Type == INDEX)
    {
      Node *n = root->Children[0]->Children[0];
      int i = 0;
      while (
          i < root->Children[0]->Children.size() && n->Type != INDEXEND)
      {
  ...
  void CompileEqual16(Node *root)
  {
    ...
    if (root->Children[0]->Type == REFERENCE)
    {
      ...
      {
        if (
            root->Children[0]->Children[0]->Type == INDEX && root->Children[0]->Children[0]->Children[0]->NodeToken->Type == CONSTANT)
        {
    ...
    if (root->Parent->Type == INDEXEND)
    {
      Node *n = root;
      while (
          n->Parent && n->Type != REFERENCE && n->Type != POINTERREFERENCE)
      {
        n = n->Parent;
      }
      ...
      if (v.pointer)
      {
        ...
        if (
            root->Children[0]->Type == CONSTANT_NODE && root->Children[0]->NodeToken->Type == STRING && root->Children[0]->NodeToken->Text.length() == 1)
          Text("mov [bx], " + ((std::string[5]){"", "byte", "word", "", "dword"})[v.size] + " '" + root->Children[0]->NodeToken->Text + "'");
        if (
            root->Children[0]->Type == CONSTANT_NODE && root->Children[0]->NodeToken->Type == STRING && root->Children[0]->NodeToken->Text.length() != 1)
        {
  ...
  {
    if (
        root->Parent->Parent->Type == NAME || root->Parent->Parent->Type == FUNCTION)
    {
